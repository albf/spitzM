...
def g_hist_ratio_trace(filename, ending, action, is_debug = False):
    ...
    for d in data_list:
        ...
        if (len(d) >= 4) and (d[0] == action): 
            ...
            #print len(d[1])
            #print d[1]
            if((len(d[1]) - d[1].index(".")) < 7):
                print 'fix1' 
                d[1] = d[1][0:d[1].index(".")+1] + ("0"* (7 - (len(d[1])) + d[1].index("."))) + d[1][(d[1].index(".")+1):len(d[1])]
                #print d[1]
            if((len(d[2]) - d[2].index(".")) < 7):
                print 'fix2'
                d[2] = d[2][0:d[2].index(".")+1] + ("0"* (7 - (len(d[2])) + d[2].index("."))) + d[2][(d[2].index(".")+1):len(d[2])]
